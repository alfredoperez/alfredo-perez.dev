...
function wikiLinkPlugin(opts = { markdownFolder: '', urlPrefix: '' }) {
      ...
      : (name) => {
          ...
          if (opts.permalinks || opts.markdownFolder) {
            const url = opts.permalinks.find(
              (p) =>
                p === name ||
                (p.split('/').pop() === name && !opts.permalinks.includes(p.split('/').pop()))
            )
            if (url) {
              if (heading) return [`${url}#${heading}`.replace(/ /g, '-').toLowerCase()]
              return [opts.urlPrefix + encodeURI(url)]
            }
          }
          return [opts.urlPrefix + encodeURI(name)]
        },
    permalinks: opts.markdownFolder
      ? getFiles(opts.markdownFolder).map((file) => file.replace('.md', ''))
      : opts.permalinks,
  }
