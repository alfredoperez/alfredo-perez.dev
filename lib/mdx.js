...
export function getFiles(type) {
  const prefixPaths = path.join(root, siteMetadata.notesPath, type === 'blog' ? '' : type)
...
export async function getFileBySlug(type, slug) {
  const mdxPath = path.join(
    root,
    type === 'blog' ? siteMetadata.notesPath : siteMetadata.authorsPath,
    `${slug}.mdx`
  )
  ...
  const { code, frontmatter } = await bundleMDX({
    ...
    xdmOptions(options, frontmatter) {
      ...
      options.remarkPlugins = [
        ...
        remarkGfm,
        remarkCodeTitles,
        [remarkObsidian, { urlPrefix: 'blog/', markdownFolder: siteMetadata.notesPath }],
...
export async function getAllFilesFrontMatter(folder) {
  const prefixPaths = path.join(root, siteMetadata.notesPath)
